...
### ✅ 坐姿阈值 30s（偏离已收口——PRD 已对齐）
- **坐姿持续阈值 = 30s**（`config.posture.hunchback_hold_ms=30000`）。原为偏离 PRD §3.2/S2-01 早期 90–120s 设想的授权偏离；2026-06-13 决策人进一步授权**把 PRD §3.2/§11 也改为 30s**，PRD 与 config 现已统一，不再是「待传播偏离」。
  残留风险：30s 短于早期 90–120s 设想，低头写字误判风险上升——**M6 彩排须专验演示中低头写字零误报**（PRD §11/§10 M6 坐姿口径）。回归更长阈值只需改 config 一处。
