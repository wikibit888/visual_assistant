...
| M2-03 | 工作记忆运行时 + memory.note/recall（原 M2-05） | A | 1.5h | 十 | 进程内 | WM 读写正确；会话结束丢弃、绝不落盘 |
...
| ID | 任务 | 负责模块 | 工时 | 依赖契约 | MOCK 并行 | 验收标准 |
|---|---|---|---|---|---|---|
| M4-01 | weather.get（Open-Meteo+缓存+写死兜底）+ 自动定位回落上海（原 M3-02） | A/工具 | 2h | weather/七 | `MOCK_WEATHER` | 真实+断网兜底+定位失败静默回落，绝不阻塞 |
...
> = 原 M5，内容不变。PRD §10/§11：彩排×3、非标准正确解法专测、模糊草稿验 unreadable、坐姿导演触发、开放对话陌生题/物体 ≥3 + 双意图诱导 + 防误锁 learning、工具序一致性专测等，本表不展开。
