...
**成本模型**：钱主要烧两处——① Live 模型的**音频 token**（一直在流）；② **多模态视觉调用**（看帧）。
原则：贵调用一律「按需触发、有硬上限、能缓存、能端侧就端侧、输出尽量短」。下表为已落地手段，阈值/档位都在 `config` 一处可调。

| 手段 | 省在哪 | 落点 |
|---|---|---|
| Live 一体（ASR+推理+TTS 一个会话）+ 不复刻 planner 编排循环 | 省三段独立计费 + 每轮额外编排调用 | 架构基线 |
| 分步靠 session memory，不每轮重传上下文 | 输入 token | PRD §4.2 |
| 视觉按需抓帧（仅 `function_call` 触发），非常开 | 不逐帧上云 | `tool_dispatch` |
| 单题视觉预算封顶 3、重试上限 1 | 抓帧/重试硬上限 | `config.session` |
| 视觉走更便宜的 `gemini-2.5-flash`（与 Live 分离） | 单价 | `config.roles.vision` |
| `check_draft` 结构化输出限 4 字段 | 输出 token 受限（且无处放答案） | `vision_tools.py` |
| 按 mode 收窄工具子集 | 少误调 + 少提示词 token | `MODE_TOOLSETS` |
| 坐姿 100% 端侧 MediaPipe | 持续监测 **零 token / 零云** | `posture.js` |
| 坐姿四闸去抖（放行+gap+冷却+去重），只在该提醒时才注入 | 少触发模型轮次 | `client_state.js` |
| 天气 Open-Meteo 无 key + 坐标/小时缓存 + 失败回落 | 零费用 + 不重复请求 | `weather.py` |
| 连接重试有上限、永久错误快失败；工具失败隔离不掀会话 | 不空耗 / 不被迫重连重计费 | `live_bridge.py` |
| 开发期 `MOCK_X=1` 脱真实依赖 | 开发/CI 零云费 | `contracts/mock.py` |
