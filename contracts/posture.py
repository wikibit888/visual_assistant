...
故 `posture.alert` 携带「发生了驼背」这一事实（单级 severity，不带话术）+ 客户端累计的
`reminder_count`（透传：计数真源在客户端 `client_state`，后端只把「第 N 次」缝进措辞）；severity 升级 /
进度措辞不在此——措辞是模型的事。
"""
...
class PostureAlert(BaseModel):
    ...
    reminder_count: int | None = Field(
        None,
        ge=1,
        description="本会话累计第几次提醒；计数真源=客户端 client_state，后端只透传缝进「第 N 次」措辞。"
        "可选（向后兼容）：缺省=客户端未带，后端落「又一次」。",
    )
