...
def planner_system_with_open_style() -> str:
    """planner system + 开放对话答复风格（措辞/组合均归 E，A 只调用不拼装，铁律：提示词权属归 E）。

    把 PLANNER_SYSTEM 与 OPEN_STYLE 串成一段交给 planner——开放对话「全交 LLM」：
    不加意图分类/路由分支（铁律4），只补「答复风格」（诚实兜底 / 期望管理 / 优雅收口）。
    连接词措辞也属 E；A 不在自己代码里拼 prompt 文本（仅调用本函数）。无路由分支、无模型名/裸阈值。
    """
    return PLANNER_SYSTEM + "\n\n# 开放对话答复风格（全交 LLM：诚实兜底 / 期望管理 / 优雅收口）\n" + OPEN_STYLE
